...
本章では@<comment>{後で置換する＠<chap>{sec-image｝}前章に引き続き、図を作成する方法、原稿に埋め込む方法、および印刷用に高解像度の画像を作成する方法を紹介します。
前章@<comment>{後で置換する＠<chap>{sec-image｝}の構成に沿っていますが、違うのはWindowsでPowerPointを使っているところです。
...
前章に倣って約束された勝利のサイクルを描いてみました@<fn>{AFFIRMED_VICTORY_TRIANGLE}。
...
@<tt>{Ctrl+S}でも保存できます。名前は前章に倣ってimages.pptxとしておきましょう。
//image[win7_draw_ppt_5][名前を付けて保存][scale=0.4]{
//}
//image[win7_draw_ppt_6][名前とファイルの種類の指定][scale=0.6]{
//}

//footnote[AFFIRMED_VICTORY_TRIANGLE][PowerPointでは円弧矢印がややこしいので、直線矢印にしました。これではサイクルというよりトライアングルですね。]
...
図形はimagesという名前のフォルダに保存し、前章のリスト1で説明されているRe:VIEW命令を使って原稿に埋め込みます。
...
これは@<comment>{後で置換する＠<chap>{sec-image｝}前章4節、リスト2を参照してください。

二つ目も@<comment>{後で置換する＠<chap>{sec-image｝}前章で紹介されている方法です。
...
三つ目は、スライドのサイズを紙面のサイズにして、狙った大きさの図を作る方法です。
 ...
 2. デザインタブのページ設定をクリックし、ページ設定ダイアログからページのサイズと向きを指定します。このとき、メニューにあるA4やB5などを選ぶと、一回り小さいサイズのスライドになります。必ず、ユーザー設定で幅と高さを指定してください（@<img>{win7_draw_ppt_18}、@<img>{win7_draw_ppt_18}）。
 3. スライドを白紙にして、図を作ります。試しにスライドの大きさをA4サイズにして先ほど作った図形を貼り付けてみると、かなり大きかった事が分かります。いい感じになるように縮小して、図として保存しましょう（@<img>{win7_draw_ppt_19}、@<img>{win7_draw_ppt_20}）。
...
前章では、三通りの方法で解像度を変更していましたが、そのうち三つ目（@<tt>{ImageMagick}の@<tt>{convert}コマンドを使う方法）はWindowsでも可能です。
...
なお、この方法で際限なく解像度を高くすることはできません。
Windows版のPowerPointにはサイズの制約があり、上限は307 DPI、3072@<m>{\times}2304ピクセルです。

//footnote[WHAT_IS_DPI][DPIの説明については前章5節を参照してください。]
...
前章でも指摘されているように、画像の解像度が高くなるとファイルサイズが増え、原稿のPDFファイルを作るのに時間がかかるようになります。
この問題に対して、前章では原稿執筆中に低解像度の画像を参照するようにし、
...
また、VSCode（Windows版バージョン1.18.1）ではフォルダへのシンボリックリンクを一つのファイルとして認識するようです。
そのため、シンボリックリンク先フォルダにあるファイルの変更は追跡してくれません。
リンク先のフォルダをフォルダを（例えば低解像度から高解像度へ）切り替えても、画像が変更されたことを認識しないということです。
