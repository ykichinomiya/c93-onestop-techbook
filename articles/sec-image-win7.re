...
三つ目は、スライドのサイズを紙面のサイズにして、狙った大きさの図を作る方法です。
 ...
 3. スライドを白紙にして、図を作ります。試しにスライドの大きさをA4サイズにして先ほど作った図形を貼り付けてみると、かなり大きかった事が分かります。いい感じになるように縮小して、図として保存しましょう（@<img>{win7_draw_ppt_20}、@<img>{win7_draw_ppt_21}）。
...
必ず標記を先に設定してください。標準の標記は16進数なので、値のデータに300を入力してから10進数に変更すると16進数の300が10進数に変換されてしまいます。
