...
この章では、Windows 7ユーザーに向けて比較的簡単なRe:VIEWのインストール方法を紹介します。
...
@<chap>{01-writing}で説明していますが、Re:VIEWはRubyと呼ばれるブログラムを介して原稿を処理します。
...
//}
//noindent
起動したコマンドプロンプト内でplatexと打ってエンターを押しみましょう。
//cmd{
> platex
//}
//noindent
...
本書の共同執筆環境でも発生します。
この章を読んでRe:VIEWの執筆環境を整えることができたので、試しに本書のリポジトリをforkしてPDFファイルを作ってみようとすると、100％遭遇します。
...
そして、（Re:VIEWの対象と思われる）TeXを知らない人にはかなりわかりにくいエラーです。
