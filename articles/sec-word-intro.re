...
ここでは、同人誌に限らず、そこそこ見栄えのよい文書を作成するために必要なWord@<fn>{WORD_VERSION}の知識を取り上げ、文書１状態から本書のスタイルを模擬して印刷できる状態にするまでの過程を簡単に説明します@<fn>{WORD_INTRO}。
...
====[column] WordドキュメントからEPUBを作る：暗黙の型宣言
