...
Keynote.appを使って、図を描いてみましょう。
 ...
 2. ウィンドウ右上の「フォーマット」タブ→「マスターを変更」ボタンを押し、「空白」を選びます。これで白紙の状態になりました。
 3. 試しに、何か適当な図を描いてみましょう。ウィンドウ上部にある「図形」アイコンをクリックし、適当な図形を選んで、位置やサイズを変更します。ここでは@<img>{keynote-screenshot}のような図を描いてみました。
 4. Command-Sを押して、スライドを保存します。保存先はプロジェクトと同じフォルダ、ファイル名は「images.key」がいいでしょう@<fn>{image-folder}。

//footnote[image-folder][Re:VIEWで使う画像は「images」フォルダに入れるので、「スライドはimages.key、画像保存先はimages」というルールが分かりやすいでしょう。]
...
//footnote[dpi2][画像のDPIを変更しても、画像の大きさは変わりません。たとえば大きさが640@<m>{\times}480である画像のDPIを72dpiから360dpiに変更しても、画像の大きさは640@<m>{\times}480のままです。]
...
画像を高解像度にすると、原稿をPDFへコンパイルするのがかなり遅くなります。正確にいうと、LaTeXファイルのコンパイル自体は変わりませんが、DVIファイル@<fn>{dvi}からPDFファイルへの変換に時間がかかるようになります。特に原稿の締め切りが近いときは、大きなストレスになります。
